  ...
  const filesJSX = fileData.map((file, index) => {
    ...
    return (
      <div className='status__files__file__container' key={index}>
        ...
        <img className={classNameCheck} src={file.error ? cross : check} alt="" />
      </div>
    );
  });
  ...
  return (
    <div className={classNameStatus}>
      <button className='status__message' onClick={handleCollapse}>
        <span>{message}</span>
        <div className={isCollapsed ? 'btn-img arrow collapsed' : 'btn-img arrow'}>
          <img className='img--arrow' src={arrowDown} alt="" />
        </div>
      </button>

      <div className={isCollapsed ? 'status__files collapsed' : 'status__files'}>
